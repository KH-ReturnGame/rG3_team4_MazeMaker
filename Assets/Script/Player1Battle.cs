...
public class Player1Battle : MonoBehaviour
{
    ...
    void Start()
    {
        Vector2Int startCell = new Vector2Int(mazeData.startX, mazeData.startY);
    ...
    void TryMove()
    {
        if (!TurnManagerBattle.Instance.CanPlayerAct()) return;
    ...
    void MoveToTarget()
    {
        ...
        if (Vector3.Distance(transform.position, targetPos) < 0.01f)
        {
            ...
            Vector2Int currentCell = WorldToGrid(transform.position);
            if (currentCell.x == mazeData.endX &&
                currentCell.y == mazeData.endY)
            {
                gameOver = true;
                Debug.Log("Player1 Win!");
                if (GameEndManager.Instance != null)
                    GameEndManager.Instance.OnPlayer1Reached();
            }
        }
    }
    ...
    public void OnOpponentReached()
    {
        if (gameOver) return;
        gameOver = true;
        Debug.Log("Player2 Win! Player1 Lose!");
    }
}